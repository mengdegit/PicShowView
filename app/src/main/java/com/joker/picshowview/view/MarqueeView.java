...
import com.joker.picshowview.utils.Utils;
...
public class MarqueeView extends ViewFlipper {
    ...
    private void startWithFixedWidth(String notice, @AnimRes int inAnimResId, @AnimRes int outAnimResID) {
        ...
        int noticeWidth = (int) paint.measureText(notice, 0, noticeLength);
        int startIndex=0;
        if (noticeWidth > width) {
            gravity = Gravity.LEFT;
            for (int index = 0; index < noticeLength; index++) {
                // 计算每一个字符的宽度
                float charWidth = paint.measureText(notice, startIndex, index);
                if (charWidth >= width) {
                    list.add(notice.substring(startIndex, index));
                    startIndex=index;
                }
            }
